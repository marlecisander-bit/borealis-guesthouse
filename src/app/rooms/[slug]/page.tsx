...
export async function generateStaticParams() { return (await contentRepository.getRooms()).map((room) => ({ slug: room.slug })); }
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params; const room = await contentRepository.getRoom(slug);
  return room ? createMetadata({ ...room.seo, image: room.image }, `/rooms/${slug}`) : { title: 'Room not found' };
}

export default async function RoomPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const [room, allRooms, experiences] = await Promise.all([contentRepository.getRoom(slug), contentRepository.getRooms(), contentRepository.getExperiences()]);
  if (!room) notFound();
  ...
  return <PublicShell mobileBooking={false}>
    ...
    <section className="shell pb-20 pt-8 md:pt-10">
      ...
      <div className="grid gap-12 py-14 lg:grid-cols-[1fr_24rem] lg:gap-20">
        ...
        <aside className="h-fit rounded-[1.75rem] bg-ivory p-7 lg:sticky lg:top-6">
          <p className="eyebrow">Book direct</p><div className="mt-5 flex items-baseline justify-between"><p className="text-sm text-muted">From</p><p className="font-serif text-4xl text-lake">€{room.priceFrom}<span className="font-sans text-xs text-muted"> / night</span></p></div>
    ...
    <RoomBookingBar slug={room.slug} price={room.priceFrom} />
  </PublicShell>;
}
