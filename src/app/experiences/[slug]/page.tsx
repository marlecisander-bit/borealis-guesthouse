...
export async function generateStaticParams() { return (await contentRepository.getExperiences()).map((item) => ({ slug: item.slug })); }
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params; const item = await contentRepository.getExperience(slug);
  return item ? createMetadata({ ...item.seo, image: item.image }, `/experiences/${slug}`) : { title: 'Experience not found' };
}

export default async function ExperiencePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const [item, rooms] = await Promise.all([contentRepository.getExperience(slug), contentRepository.getRooms()]);
  if (!item) notFound();
  ...
  if (item.priceFrom !== null) productData.offers = { '@type': 'Offer', priceCurrency: 'EUR', price: item.priceFrom, availability: 'https://schema.org/PreOrder' };
  return <PublicShell mobileBooking={false}>
      ...
      <div className="grid gap-12 py-14 lg:grid-cols-[1fr_24rem] lg:gap-20">
        <div>
          ...
          <dl className="mt-10 grid grid-cols-2 gap-5 border-y border-lake/10 py-7 sm:grid-cols-3">
            <Fact label="Duration" value={item.duration}/><Fact label="Capacity" value={item.capacity}/><Fact label="Starting price" value={item.priceFrom===null?item.priceType:`€${item.priceFrom}`} />
          </dl>
        ...
        <aside className="h-fit rounded-[1.75rem] bg-lake p-7 text-white lg:sticky lg:top-6">
          <p className="text-xs font-bold uppercase tracking-widest text-sand">Plan this experience</p><p className="mt-5 text-sm text-white/60">{item.priceFrom===null?item.priceType:'Starting from'}</p>{item.priceFrom!==null&&<p className="mt-1 font-serif text-4xl">€{item.priceFrom}</p>}<p className="mt-5 text-sm leading-6 text-white/65">Mock product details only. Availability and operating details will be confirmed before launch.</p>
          <Link href={`/book?experience=${item.slug}&mode=experience`} className="mt-6 block min-h-14 rounded-xl bg-sand px-5 py-4 text-center text-sm font-bold text-lake">Book independently</Link>
          <Link href={`/book?addon=${item.slug}`} className="mt-3 block min-h-14 rounded-xl border border-white/25 px-5 py-4 text-center text-sm font-bold text-white">Add to a room stay</Link>
        </aside>
      </div>
    </section>
    ...
    <ExperienceActions slug={item.slug} price={item.priceFrom} />
  </PublicShell>;
}
