...
function RoomList({rooms}:{rooms:AdminRoomType[]}){
  return <div className="overflow-visible rounded-2xl border border-slate-200 bg-white md:overflow-x-auto">
    ...
    {rooms.map(room=><article key={room.id} className="admin-room-type-grid grid gap-4 border-b border-slate-100 p-5 last:border-0 md:min-w-[68rem] md:items-center">
      <div className="relative aspect-[16/9] overflow-hidden rounded-lg bg-slate-100 md:aspect-[4/3]">{room.images[0]?<Image src={room.images.find(item=>item.isFeatured)?.publicUrl||room.images[0].publicUrl} alt="" fill sizes="(max-width: 767px) calc(100vw - 3.5rem), 80px" className="object-cover"/>:<span className="absolute inset-0 grid place-items-center text-xs text-slate-400">No image</span>}</div>
      ...
      <div className="grid grid-cols-2 items-start gap-2 md:flex md:flex-wrap md:items-center"><Link href={`/admin/rooms/${room.id}`} className="inline-flex min-h-11 items-center justify-center rounded-lg border border-slate-300 px-3 text-center text-xs font-bold leading-none">Edit</Link>{room.status!=='archived'&&<><QuickRoomRateForm roomId={room.id} roomName={room.name} price={room.basePrice} currency={room.currency} active={room.rateActive}/><Link href="/admin/availability#block-dates" className="inline-flex min-h-11 items-center justify-center rounded-lg border border-slate-300 px-3 text-center text-xs font-bold leading-none">Availability</Link></>}{room.status==='archived'?<form action={restoreRoomType.bind(null,room.id)}><button className="inline-flex min-h-11 w-full items-center justify-center rounded-lg px-3 text-center text-xs font-bold leading-none text-emerald-700">Restore</button></form>:<details className="relative max-md:w-full"><summary className="inline-flex min-h-11 w-full cursor-pointer list-none items-center justify-center rounded-lg px-3 text-xs font-bold">More</summary><div className="mt-2 grid w-full gap-1 rounded-xl border border-slate-200 bg-white p-2 shadow-xl md:absolute md:right-0 md:z-10 md:w-40"><form action={duplicateRoomType.bind(null,room.id)}><button className="min-h-11 w-full rounded-lg px-3 text-left text-xs font-bold hover:bg-slate-50">Duplicate</button></form><ConfirmAction action={archiveRoomType.bind(null,room.id)} label="Archive" confirmMessage={`Archive ${room.name}? It will be removed from the website and booking availability.`}/></div></details>}</div>
    </article>)}
  </div>;
}
