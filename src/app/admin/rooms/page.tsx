...
export default async function RoomsPage({searchParams}:{searchParams:Promise<{status?:string}>}){
  ...
  return <div className="space-y-8">
    <AdminPageHeader title="Rooms" description="Manage every guest-facing room, its inventory and its booking details in one place." actions={<Link href="/admin/rooms/new" className="inline-flex min-h-11 items-center justify-center rounded-lg bg-slate-950 px-5 text-center text-sm font-semibold text-white">Add Room</Link>}/>
...
function RoomList({rooms}:{rooms:AdminRoomType[]}){
  return <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white">
    ...
    {rooms.map(room=><article key={room.id} className="admin-room-type-grid grid gap-4 border-b border-slate-100 p-5 last:border-0 md:min-w-[68rem] md:items-center">
      ...
      <div><Link href={`/admin/rooms/${room.id}`} className="font-bold text-slate-950 hover:underline">{room.name}</Link><p className="mt-1 text-xs text-slate-500">{room.slug}{room.featured?' · Featured':''}</p></div>
      <div><p className="text-sm text-slate-700">{room.capacity} {room.capacity===1?'guest':'guests'}</p><p className="mt-1 text-xs text-slate-500">Inventory: {room.inventoryCount}</p></div>
      <p className="text-sm font-semibold text-slate-900">{formatRoomRate(room.basePrice,room.currency)}</p>
      <StatusBadge status={room.status}/>
      <div className="flex flex-wrap items-center gap-2"><Link href={`/admin/rooms/${room.id}`} className="inline-flex min-h-11 items-center justify-center rounded-lg border border-slate-300 px-3 text-center text-xs font-bold leading-none">Edit</Link>{room.status==='archived'?<form action={restoreRoomType.bind(null,room.id)}><button className="inline-flex min-h-11 items-center justify-center rounded-lg px-3 text-center text-xs font-bold leading-none text-emerald-700">Restore</button></form>:<><form action={duplicateRoomType.bind(null,room.id)}><button className="inline-flex min-h-11 items-center justify-center rounded-lg px-3 text-center text-xs font-bold leading-none">Duplicate</button></form><ConfirmAction action={archiveRoomType.bind(null,room.id)} label="Archive" confirmMessage={`Archive ${room.name}? It will be removed from public content and availability.`}/></>}</div>
    </article>)}
  </div>;
}
