...
export default async function ExperiencesPage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  ...
  return <div className="space-y-8">
    ...
    <form className="flex flex-wrap items-end gap-3 rounded-xl border border-slate-200 bg-white p-4"><label className="text-xs font-bold text-slate-600">Show<select name="status" defaultValue={status} className="ml-2 min-h-10 rounded-lg border border-slate-300 px-3"><option value="active">Current experiences</option><option value="archived">Archived experiences</option></select></label><button className="min-h-10 rounded-lg bg-slate-100 px-4 text-sm font-bold">Filter</button></form>
    {filtered.length ? <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">{filtered.map(item => <article key={item.id} className="grid gap-4 border-b border-slate-100 p-5 last:border-0 md:grid-cols-[5rem_1.4fr_.7fr_.7fr_auto] md:items-center">
      ...
      <div className="flex flex-wrap gap-2"><Link href={`/admin/experiences/${item.id}`} className="inline-flex min-h-11 items-center justify-center rounded-lg border border-slate-300 px-3 text-xs font-bold">Edit</Link>{item.status === 'archived' ? <form action={restoreExperience.bind(null, item.id)}><button className="min-h-11 px-3 text-xs font-bold text-emerald-700">Restore</button></form> : <><form action={duplicateExperience.bind(null, item.id)}><button className="min-h-11 px-3 text-xs font-bold">Duplicate</button></form>{item.status === 'published' && <form action={unpublishExperience.bind(null, item.id)}><button className="min-h-11 px-3 text-xs font-bold">Unpublish</button></form>}<ConfirmAction action={archiveExperience.bind(null, item.id)} label="Archive" confirmMessage={`Archive ${item.name}?`}/></>}</div>
