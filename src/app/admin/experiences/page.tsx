...
export default async function ExperiencesPage({ searchParams }: { searchParams: Promise<{ status?: string;page?:string }> }) {
  const session = await requireAdmin();
  const { status = 'active',page='1' } = await searchParams;
  const data=await adminExperiencesRepository.summaries(session,Number(page),status);
  const filtered=data.items;

  return <div className="space-y-8">
    <AdminPageHeader title="Experiences" description="Manage activities guests can discover and book." actions={<Link prefetch={false} href="/admin/experiences/new" className="inline-flex min-h-11 items-center justify-center rounded-lg bg-slate-950 px-4 text-sm font-semibold text-white">Add experience</Link>}/>
    <Form action="" className="grid gap-3 rounded-xl border border-slate-200 bg-white p-4 min-[430px]:grid-cols-[minmax(0,1fr)_auto] min-[430px]:items-end"><label className="text-xs font-bold text-slate-600">Show<select name="status" defaultValue={status} className="mt-2 min-h-11 w-full rounded-lg border border-slate-300 px-3"><option value="active">Current experiences</option><option value="archived">Archived experiences</option></select></label><button className="min-h-11 rounded-lg bg-slate-100 px-4 text-sm font-bold">Filter</button></Form>
    ...
    <Pagination page={data.page} pageSize={data.pageSize} total={data.total} pathname="/admin/experiences" filters={{status}}/>
  </div>;
}
