...
export async function getDatabaseArticles(includeId?:string):Promise<TourismArticle[]>{
  try{
    ...
    const{data,error}=await query;if(error||!data?.length)return[];const rows=data as Row[],rowIds=rows.map(row=>row.id);
