...
export async function getDatabaseTransfers():Promise<TransferRoute[]>{
  try{
    const db=await createServerSupabaseClient(),base=process.env.NEXT_PUBLIC_SUPABASE_URL||'';
    const{data,error}=await db.from('transfer_routes').select('id,origin,destination,slug,short_description,description,full_description,cover_media_id,duration_label,duration_minutes,capacity,service_type,price,currency,price_type,booking_notice,active,is_featured,bookable,availability_mode,available_days,window_start,window_end').eq('status','published').eq('is_visible',true).eq('active',true).order('is_featured',{ascending:false}).order('sort_order');
