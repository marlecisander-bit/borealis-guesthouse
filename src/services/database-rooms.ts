...
type TypeRow={id:string;slug:string;name:string;short_description:string|null;long_description:string|null;capacity:number;max_adults:number|null;max_children:number|null;max_infants:number|null;max_total_occupancy:number|null;min_adults:number|null;infants_count_toward_capacity:boolean|null;bed_configuration:string|null;size_sqm:number|null;view_type:string|null;is_featured:boolean};
...
export const getDatabaseRooms=cache(async(includeId?:string):Promise<Room[]>=>{
  ...
  try{
    ...
    return(types as TypeRow[]).map(row=>{
      ...
      const price=prices.get(row.id);const meta=((seo||[]) as SeoRow[]).find(item=>item.entity_id===row.id);const ids=(links||[]).filter(item=>item.room_type_id===row.id).map(item=>item.amenity_id);const maxTotal=row.max_total_occupancy??row.capacity;
      return{id:row.id,slug:row.slug,name:row.name,eyebrow:row.is_featured?'Featured room':'Stay at Borealis',description:row.short_description||'',longDescription:row.long_description||row.short_description||'',image:roomImages[0]||images.room,gallery:roomImages.length?roomImages:[images.room],priceFrom:price?.amount??null,currency:price?.currency??'EUR',capacity:maxTotal,occupancyPolicy:{maxAdults:row.max_adults??maxTotal,maxChildren:row.max_children??0,maxInfants:row.max_infants??0,maxTotalOccupancy:maxTotal,minAdults:row.min_adults??1,infantsCountTowardCapacity:row.infants_count_toward_capacity??true},beds:row.bed_configuration||'Bed details available on request',size:row.size_sqm?`${row.size_sqm} m²`:'Size available on request',viewType:row.view_type||'Borealis view',amenities:(amenities||[]).filter(item=>ids.includes(item.id)).map(item=>({id:item.id,name:item.name,icon:item.icon||undefined})),seo:{title:meta?.title||row.name,description:meta?.meta_description||row.short_description||row.name,ogTitle:meta?.og_title||undefined,ogDescription:meta?.og_description||undefined,canonical:meta?.canonical_override||undefined,noindex:meta?.noindex||false}};
    });
  }catch{return[]}
});
