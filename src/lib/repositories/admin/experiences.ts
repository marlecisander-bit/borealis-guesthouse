...
export const adminExperiencesRepository = {
  async list(session:AdminSession):Promise<AdminExperience[]> {
    ...
    const [{data,error},{data:images},{data:media},{data:seo}]=await Promise.all([
      db.from('experiences').select('id,name,slug,short_description,full_description,cover_media_id,duration_label,duration_minutes,max_capacity,price,currency,pricing_type,meeting_point,included_items,excluded_items,important_notes,booking_notice,active,is_featured,bookable,book_independently,minimum_quantity,maximum_quantity,booking_cutoff_hours,operating_days,operating_start_time,operating_end_time,slot_interval_minutes,active_date_start,active_date_end,status,sort_order,availability_mode,updated_at').eq('property_id',session.propertyId).order('sort_order'),
      db.from('experience_images').select('experience_id,media_asset_id,sort_order').eq('property_id',session.propertyId).neq('status','archived').order('sort_order'),
      db.from('media_assets').select('id,file_path').eq('property_id',session.propertyId).neq('status','archived'),
      db.from('seo_metadata').select('entity_id,title,meta_description').eq('property_id',session.propertyId).eq('entity_type','experience'),
    ]);
