...
export function SettingsForm({ data }: { data: SettingsDashboard }) {
  ...
  return <form action={action} className="space-y-6">
    ...
    <Section title="Content settings">
      <div className="grid gap-5 sm:grid-cols-2"><Field label="Default language"><select name="defaultLanguageId" defaultValue={s.defaultLanguageId} className={input}><option value="">No language configured</option>{data.languages.map(x => <option key={x.id} value={x.id}>{x.label}</option>)}</select></Field><Field label="Default SEO image"><select name="defaultSeoMediaId" defaultValue={s.defaultSeoMediaId} className={input}><option value="">No default image</option>{data.media.map(x => <option key={x.id} value={x.id}>{x.label}</option>)}</select></Field></div>
      <Field label="Default contact CTA"><input name="defaultContactCta" defaultValue={s.defaultContactCta} className={input}/></Field>
    </Section>
