...
export function AdminWorkspaceNav() {
  ...
  return <nav aria-label={`${workspace.label} sections`} className="mb-6 overflow-x-auto rounded-xl border border-[#d5e5e2] bg-white p-1.5 shadow-sm"><div className="flex min-w-max gap-1">{workspace.tabs.map((tab) => {const active=matchesPath(pathname,[tab.href,...(tab.matches||[])]);return <Link prefetch={false} key={tab.href} href={tab.href} aria-current={active?'page':undefined} className={`inline-flex min-h-10 items-center rounded-lg px-3.5 text-sm font-semibold transition ${active?'bg-[#164b59] text-white':'text-[#45656a] hover:bg-[#e8f4f3] hover:text-[#164b59]'}`}>{tab.label}</Link>})}</div></nav>;
}
