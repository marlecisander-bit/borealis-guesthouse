...
export function MediaUploader({ onUploaded, compact = false }: {
  ...
  return <div className={compact ? 'space-y-3' : 'rounded-2xl border border-slate-200 bg-white p-5 sm:p-6'}>
    ...
    <div className="mt-4 flex flex-wrap items-center gap-3"><button type="button" disabled={busy || readyCount === 0} onClick={() => void upload()} className="min-h-11 rounded-lg bg-slate-950 px-5 text-sm font-semibold text-white disabled:opacity-50">{busy ? 'Uploading photos…' : readyCount ? `Upload ${readyCount} image${readyCount === 1 ? '' : 's'}` : 'Upload images'}</button>{!busy && retryableFailed.length > 0 && <button type="button" onClick={() => void upload(retryableFailed.map(item => item.id))} className="min-h-11 rounded-lg border border-slate-300 px-4 text-sm font-semibold">Retry failed</button>}{message && <p role="status" aria-live="polite" className="text-sm text-slate-600">{message}</p>}</div>
